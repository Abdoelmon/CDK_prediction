...
biopro = pd.read_sas("BIOPRO_I.XPT", format='xport')[['SEQN', 'LBXSCR']]
alb_cr = pd.read_sas("ALB_CR_I.XPT", format='xport')[['SEQN', 'URXUMA', 'URXUCR']]
ghb = pd.read_sas("GHB_I.XPT", format='xport')[['SEQN', 'LBXGH']]
cbc = pd.read_sas("CBC_I.XPT", format='xport')[['SEQN', 'LBXHGB']]
bpx = pd.read_sas("BPX_I.XPT", format='xport')[['SEQN', 'BPXDI1', 'BPXSY1']]  # Adjusted for 2015–2016 variable names
bmx = pd.read_sas("BMX_I.XPT", format='xport')[['SEQN', 'BMXBMI', 'BMXWAIST']]
diq = pd.read_sas("DIQ_I.XPT", format='xport')[['SEQN', 'DIQ010']]
mcq = pd.read_sas("MCQ_I.XPT", format='xport')[['SEQN', 'MCQ160B', 'MCQ160C']]
smq = pd.read_sas("SMQ_I.XPT", format='xport')[['SEQN', 'SMQ020']]
...
# Save
merged.to_csv("nhanes_ckd_2015_2016.csv", index=False)
