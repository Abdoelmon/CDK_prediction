...
biopro = pd.read_sas("BIOPRO_D.XPT", format='xport')[['SEQN', 'LBXSCR']]
alb_cr = pd.read_sas("ALB_CR_D.XPT", format='xport')[['SEQN', 'URXUMA', 'URXUCR']]
ghb = pd.read_sas("GHB_D.XPT", format='xport')[['SEQN', 'LBXGH']]
cbc = pd.read_sas("CBC_D.XPT", format='xport')[['SEQN', 'LBXHGB']]
bpx = pd.read_sas("BPX_D.XPT", format='xport')[['SEQN', 'BPXDI1', 'BPXSY1']]  # Adjusted for 2005–2006 variable names
bmx = pd.read_sas("BMX_D.XPT", format='xport')[['SEQN', 'BMXBMI', 'BMXWAIST']]
diq = pd.read_sas("DIQ_D.XPT", format='xport')[['SEQN', 'DIQ010']]
mcq = pd.read_sas("MCQ_D.XPT", format='xport')[['SEQN', 'MCQ160B', 'MCQ160C']]
smq = pd.read_sas("SMQ_D.XPT", format='xport')[['SEQN', 'SMQ020']]
...
# Save
merged.to_csv("nhanes_ckd_2005_2006.csv", index=False)
