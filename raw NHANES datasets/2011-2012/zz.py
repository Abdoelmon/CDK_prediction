...
biopro = pd.read_sas("BIOPRO_G.XPT", format='xport')[['SEQN', 'LBXSCR']]
alb_cr = pd.read_sas("ALB_CR_G.XPT", format='xport')[['SEQN', 'URXUMA', 'URXUCR']]
ghb = pd.read_sas("GHB_G.XPT", format='xport')[['SEQN', 'LBXGH']]
cbc = pd.read_sas("CBC_G.XPT", format='xport')[['SEQN', 'LBXHGB']]
bpx = pd.read_sas("BPX_G.XPT", format='xport')[['SEQN', 'BPXSY1', 'BPXDI1']]  # Adjusted for 2011–2012 variable names
bmx = pd.read_sas("BMX_G.XPT", format='xport')[['SEQN', 'BMXBMI', 'BMXWAIST']]
diq = pd.read_sas("DIQ_G.XPT", format='xport')[['SEQN', 'DIQ010']]
mcq = pd.read_sas("MCQ_G.XPT", format='xport')[['SEQN', 'MCQ160B', 'MCQ160C']]
smq = pd.read_sas("SMQ_G.XPT", format='xport')[['SEQN', 'SMQ020']]
...
# Save
merged.to_csv("nhanes_ckd_2011_2012.csv", index=False)
