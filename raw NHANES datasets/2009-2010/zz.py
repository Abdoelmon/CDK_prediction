...
# Load datasets
demo = pd.read_sas("DEMO_F.XPT", format='xport')[['SEQN', 'RIDAGEYR', 'RIAGENDR', 'RIDRETH1']]
biopro = pd.read_sas("BIOPRO_F.XPT", format='xport')[['SEQN', 'LBXSCR']]
alb_cr = pd.read_sas("ALB_CR_F.XPT", format='xport')[['SEQN', 'URXUMA', 'URXUCR']]
ghb = pd.read_sas("GHB_F.XPT", format='xport')[['SEQN', 'LBXGH']]
cbc = pd.read_sas("CBC_F.XPT", format='xport')[['SEQN', 'LBXHGB']]
bpx = pd.read_sas("BPX_F.XPT", format='xport')[['SEQN', 'BPXSY1', 'BPXDI1']]  # Adjusted for 2009–2010 variable names
bmx = pd.read_sas("BMX_F.XPT", format='xport')[['SEQN', 'BMXBMI', 'BMXWAIST']]
diq = pd.read_sas("DIQ_F.XPT", format='xport')[['SEQN', 'DIQ010']]
mcq = pd.read_sas("MCQ_F.XPT", format='xport')[['SEQN', 'MCQ160B', 'MCQ160C']]
smq = pd.read_sas("SMQ_F.XPT", format='xport')[['SEQN', 'SMQ020']]
...
# Save
merged.to_csv("nhanes_ckd_2009_2010.csv", index=False)
