...
biopro = pd.read_sas("BIOPRO_E.XPT", format='xport')[['SEQN', 'LBXSCR']]
alb_cr = pd.read_sas("ALB_CR_E.XPT", format='xport')[['SEQN', 'URXUMA', 'URXUCR']]
ghb = pd.read_sas("GHB_E.XPT", format='xport')[['SEQN', 'LBXGH']]
cbc = pd.read_sas("CBC_E.XPT", format='xport')[['SEQN', 'LBXHGB']]
bpx = pd.read_sas("BPX_E.XPT", format='xport')[['SEQN', 'BPXSY1', 'BPXDI1']]  # Adjusted for 2007–2008 variable names
bmx = pd.read_sas("BMX_E.XPT", format='xport')[['SEQN', 'BMXBMI', 'BMXWAIST']]
diq = pd.read_sas("DIQ_E.XPT", format='xport')[['SEQN', 'DIQ010']]
mcq = pd.read_sas("MCQ_E.XPT", format='xport')[['SEQN', 'MCQ160B', 'MCQ160C']]
smq = pd.read_sas("SMQ_E.XPT", format='xport')[['SEQN', 'SMQ020']]
...
# Save
merged.to_csv("nhanes_ckd_2007_2008.csv", index=False)
